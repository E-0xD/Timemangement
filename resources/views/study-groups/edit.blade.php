<x-layouts::app>
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl p-4 md:p-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-semibold text-zinc-900 dark:text-zinc-100">Edit Group</h1>
                <p class="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">{{ $group->name }}</p>
            </div>
        </div>
        ...
        <div class="mx-auto w-full max-w-2xl space-y-4">
            ...
            <div class="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                <form method="POST" action="{{ route('groups.update', $group) }}" id="update-group-form">
                    @csrf @method('PUT')

                    @include('study-groups.partials.form-fields', ['model' => $group])
            ...
            @if($group->owner_id === Auth::id())
                <div class="rounded-xl border border-red-200 bg-white p-5 shadow-sm dark:border-red-900 dark:bg-zinc-900">
                    <h3 class="mb-1 text-sm font-semibold text-red-700 dark:text-red-400">Delete Group</h3>
                    <p class="mb-4 text-xs text-zinc-500 dark:text-zinc-400">This will permanently delete the group and all its messages.</p>
                    <form method="POST" action="{{ route('groups.destroy', $group) }}" id="delete-group-form">
                        @csrf @method('DELETE')
                        <button type="submit" form="delete-group-form"
